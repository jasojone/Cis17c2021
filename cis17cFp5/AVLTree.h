...
public:
    ...
    BNTnode* insert(BNTnode*, int, string);//Insert and balance the tree
    ...
    int search(BNTnode* root, int padDigit, string init); // BFS search function
};
...
//******************************************************************************
BNTnode* AVLTree::insert(BNTnode* root, int value, string init) {
    if (root == NULL) {
        root = new BNTnode;
        root->data = value;
        root->initials = init;
...
//******************************************************************************
BNTnode* AVLTree::delNode(BNTnode* root, int x) {
    //standard delete
    if (root == NULL)
        return root;
...
//******************************************************************************
void AVLTree::levelOrder(BNTnode* root) {
    // Base Case
    if (root == NULL)  return;
...
int AVLTree::search(BNTnode* root, int padDigit, string init)
{
    // Base Case
    if (root == NULL) 
        return 0;
    ...
    while (q.empty() == false)
    {
        ...
        if (node->initials == init && node->data == padDigit)
            return q.size();
